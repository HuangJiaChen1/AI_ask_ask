...
### Dual-Parallel Generation (v3.0+)

**Core Concept:** Decoupling the "Pedagogical Response" (feedback/explanation) from the "Next Question" logic to prevent instruction leakage.

```mermaid
sequenceDiagram
    participant Orchestrator as orchestrate_dual_stream
    participant Stream1 as Response Stream
    participant Stream2 as Question Stream
    participant User as Frontend

    Orchestrator->>Stream1: Start Task (Feedback Prompt)
    Orchestrator->>Stream2: Start Task (Focus Prompt)
    
    par Parallel Execution
        Stream1->>Stream1: Generate Feedback
        Stream2->>Stream2: Generate Question
    end

    loop Serial Consumption
        Stream1-->>User: Stream Feedback Chunks
    end

    Orchestrator-->>User: Yield Bridge ("\n\n")

    loop Serial Consumption
        Stream2-->>User: Stream Question Chunks
    end
```

**Why This Matters:**
- **Zero Leakage:** The logic explaining "Width-Color" won't accidentally ask a "Depth" question because they are generated by separate prompts in separate contexts.
- **Strict Adherence:** The Question Stream focuses 100% on the Focus Mode strategy.
- **Natural Flow:** The user sees a seamless stream, but under the hood, it's two orchestrated intelligences working in parallel.
...
### Version 3.0 (January 2026): Dual-Parallel Generation Architecture

**Revolutionary Change:** Decoupled the "Response" generation from "Follow-up Question" generation using parallel API calls.

**What Changed:**
1. ✅ **Decoupled Logic:** Feedback/Explanation logic is now physically separated from Question Generation logic.
2. ✅ **Parallel Execution:** Two API calls run simultaneously to minimize latency.
3. ✅ **Strict Focus Adherence:** The follow-up question is generated by a dedicated prompt that strictly follows the Focus Mode, eliminating "instruction leakage" where explanations influenced the next question type.
4. ✅ **Robust "I Don't Know" Handling:** Flexible handling for factual vs. open-ended "I don't know" responses.

**Technical Changes:**
- **Refactored**: `paixueji_stream.py` to use `orchestrate_dual_stream`.
- **New Prompts**: Split monolithic prompts into `FEEDBACK_RESPONSE_PROMPT` and `FOLLOWUP_QUESTION_PROMPT`.
